...
**PixiJS Canvas Integration Implementation (2026-02-17):** Implemented Phase 1 MVP of PixiJS game canvas. Created GameCanvas.tsx (React wrapper managing PixiJS Application lifecycle, refs for app/sceneManager, useEffect hooks for initialization/resize/state sync) and SceneManager.ts (pure PixiJS class, layered containers for z-order, sprite registry Map<EntityId, Container> for incremental updates, pixi-viewport for pan/zoom). Pattern: State flows Colyseus → store → React props → SceneManager.updateWorld(), interactions flow PixiJS events → callbacks → React setState. Node rendering: Graphics circles colored by player.color (hex → int), selection ring on click, Text labels. Connection rendering: batch Graphics.moveTo/lineTo for all connections. Type compatibility: pixi-viewport v5 with pixi.js v8 requires type assertions (`as any`) for Viewport construction and addChild calls - functionally correct, types misaligned. Integration: GameCanvas replaces placeholder in GameWorld.tsx, handleNodeClick wrapper (nodeId: EntityId for canvas, node: Node for NodeView grid). Files: src/components/GameCanvas/{GameCanvas.tsx, SceneManager.ts, GameCanvas.module.css, index.ts}. PR #11 created, all tests passing.

**PixiJS Canvas Visibility Fix (2026-02-17):** Fixed viewport centering and CSS height issues preventing canvas visibility. Viewport was centered at (1000, 1000) but nodes positioned in 200-700 range — changed to moveCenter(450, 350) to match actual game coordinates. CSS issue: `.canvas` container used `min-height: 300px` which broke flex container height calculation — changed to `min-height: 0` (flex container best practice to allow proper shrinking). Pattern: Viewport positioning should match actual game coordinate space, not arbitrary values. Flex containers need `min-height: 0` to properly respect flex: 1 sizing. File locations: SceneManager.ts (line 105), GameWorld.module.css (.canvas rule).
